...
namespace App\Filament\Resources\PaymentMethodResource\Widgets;
...
use Filament\Forms;
use Carbon\Carbon;
use App\Filament\Resources\PaymentMethodResource\Widgets\PaymentMethodStatsExport;

class PaymentMethodStatsWidget extends BaseWidget
{
    protected int | string | array $columnSpan = 'full';
    protected int | string | array $columns = ['default' => 1, 'md' => 2];
    ...
    public function mount()
    {
        ...
        if (!isset($this->filters['show_details'])) {
            $this->filters['show_details'] = false;
        }

        // Ensure month is properly formatted
        $this->filters['month'] = str_pad($this->filters['month'], 2, '0', STR_PAD_LEFT);
    }

    protected function validateFilters(): void
    {
        // Ensure year is valid
        if (!is_numeric($this->filters['year'])) {
            $this->filters['year'] = date('Y');
        }
        ...
        $this->filters['show_details'] = (bool) ($this->filters['show_details'] ?? false);
    }
    ...
    protected function getFilters(): ?array
    {
        ...
        return [
            'year' => Forms\Components\Select::make('year')
                ->label('Tahun')
                ->options($years->mapWithKeys(fn($year) => [$year => $year])->toArray())
                ->default(date('Y')),
            'month' => Forms\Components\Select::make('month')
                ->label('Bulan')
                ->options([
                    '01' => 'Januari',
                    '02' => 'Februari', 
                ...
                ->default(date('m')),
            'show_details' => Forms\Components\Toggle::make('show_details')
    ...
    protected function getStats(): array
    {
        ...
        $month = $this->filters['month'];
        $showDetails = $this->filters['show_details'];
